 ...
 *         ];
 *        
 *         $ts = new bioorm\Core\SchemaBuilder($schema);
 *         $sql = $ts->create("user");
 */
class SchemaBuilder
{
    ...
    protected function addColumn(Array $properties)
    {
        ...
        $properties["unsigned"] = ($properties["unsigned"] && $this->isTypeNumeric($properties["type"])) ? "UNSIGNED" : "";
        $properties["auto_increment"] = ($properties["auto_increment"] && $this->isTypeNumeric($properties["type"])) ? "AUTO_INCREMENT" : "";
        $column = [];
